      ...
      <div className="space-y-3">
        <div className="flex space-x-2">
          ...
          <button
            type="button"
            onClick={onChange}
            className="px-6 py-3 bg-purple-600 text-white font-medium rounded-lg hover:bg-purple-700 transition-colors duration-200 whitespace-nowrap"
          >
            주소 검색
          </button>
        </div>
    ...
    if (window.daum && window.daum.Postcode) {
      new window.daum.Postcode({
        oncomplete: function (data) {
          ...
          // 상세주소 입력 필드에 포커스
          setTimeout(() => {
            const detailInput = document.querySelector('input[name="addressDetail"]');
            if (detailInput) {
              detailInput.focus();
            }
          }, 100);
